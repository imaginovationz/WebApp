...
if not any(isinstance(h,logging.StreamHandler)for h in werk.handlers):
...
def get_db_connection():
    conn = mysql.connector.connect(
        host='10.239.43.100',         # or your MySQL host
        #user='nigam',   # your MySQL username
        #password='nigampwd', # your MySQL password
        
        user='root',   # your MySQL username
        password='root', # your MySQL password
         database='cbpt_automation_db',  # your database name
        #port: 3306 
    )
    return conn
    ...
    try:
        ...
        if release:
            sql = """
              ...
              ORDER BY intake_name COLLATE utf8mb4_general_ci
                 LIMIT %s
            """
            #rows = cursor.execute(sql, (release, limit)).fetchall()
            cursor.execute(sql, (release, limit))
        ...
        sql = """
            SELECT intake_number, intake_name
            FROM projects
            WHERE intake_number LIKE %s OR intake_name LIKE %s
            LIMIT %s
        """
        like_q = f"%{q}%"
        ...
        cursor.execute(sql, (like_q, like_q, limit))
        rows = cursor.fetchall()
...
def get_project_details(intake_number):
    ...
    try:
        ...
        cur.execute(
            "SELECT * FROM projects WHERE intake_number = %s ORDER BY timestamp DESC LIMIT 1",
            (intake_number,)
        )
        #cursor.execute(sql, (like_q, like_q, limit))
        row = cur.fetchone()
        ...
        try:
            ...
            if cur.fetchall():
                cur.execute(
                    "SELECT ImplementationDate FROM projects_timeline WHERE intake_number = %s ORDER BY ROWID DESC LIMIT 1",
                    (intake_number,)
                )
                row = cur.fetchone()

                if row:
                    data['ImplementationDate'] = row['ImplementationDate'] if isinstance(row, sqlite3.Row) else row[0]
        except Exception:
            pass
...
def get_automation_deliverables(intake_number):
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT * FROM automationdeliverables WHERE intake_number = %s ORDER BY ROWID DESC LIMIT 1",
            (intake_number,)
        )
        row = cur.fetchone()
        conn.close()
...
def resourcefetch():
    try:
        ...
        if name:
            cur.execute("SELECT email FROM resources WHERE name = %s LIMIT 1;", (name,))
            row = cur.fetchone()
            conn.close()
            if not row:
...
def fetch_application():
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute(
           "SELECT application FROM application WHERE domain = %s ORDER BY application COLLATE NOCASE;",
            (domain,)
        )
        rows = cur.fetchall()
        conn.close()
...
def projectmasterupdate():
    ...
    def _upsert_initiatives(conn, intake_number, col, val, intake_name=None):
        ...
        # ensure a row exists
        r = cur.execute(
            "SELECT 1 FROM InitiativesPlannedUsage WHERE intake_number = %s LIMIT 1",
            (intake_number,)
        ).fetchone()
        if not r:
            cur.execute(
                "INSERT INTO InitiativesPlannedUsage (intake_number, intake_name) VALUES (%s, %s)",
                (intake_number, intake_name or "")
            )
        # update the single column
        cur.execute(f'UPDATE InitiativesPlannedUsage SET "{col}" = ? WHERE intake_number = %s',
                    (val, intake_number))
        conn.commit()
    ...
    allowed = {
        "projects": {
            ...
            "EstimateReviewedBy","FuncAutomationScope","AutoOOSReason",
    ...
    try:
        ...
        if pt_cols and pt_update:
            ...
            if key_col:
                vals = list(pt_update.values()) + [intake_number]
                cur.execute(f"UPDATE projects_timeline SET {set_clause} WHERE {key_col}=?", vals)
                if cur.rowcount == 0:
                    cols_with_key = [key_col] + list(pt_update.keys())
                    placeholders = ",".join(["?"] * len(cols_with_key))
                    vals_ins = [intake_number] + list(pt_update.values())
                    cur.execute(
...
def fetch_releases():
    ...
    try:
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("SELECT release_name FROM releases ORDER BY release_name COLLATE utf8mb4_general_ci")
        rows = cur.fetchall()
        conn.close()
