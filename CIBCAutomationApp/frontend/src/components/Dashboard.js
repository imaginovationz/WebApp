...
const ResourceUtilizationAllocation = ({ chartType, data }) => {
  ...
  const [loading, setLoading] = useState(false); 
    ...
    useEffect(() => {
        if (data) {
            ...
            // Extract the colors as flat arrays for easier use
            ...
            const datasets = data.reduce((acc, item) => {
                const label = `${item.resource_name} (${item.intake_number})`;
            ...
            const pieData = {
                ...
                datasets: [{
                    ...
                    backgroundColor: datasetLabels.map((_, i) => colorPalette[i % colorPalette.length].bg),
                    borderColor: datasetLabels.map((_, i) => borderColors[i % borderColors.length]),
                    borderWidth: 2,
    ...
    const chartOptions = {
        ...
        plugins: {
            ...
            title: {
                display: true,
                //text: "Weekly Resource On Intake",
                font: {
                    size: 18,
                    family: "'Poppins', 'Roboto', sans-serif",
                    weight: '700'
...
const ResourceUtilizationCapacity = ({ chartType, data}) => {
  const [chartData, setChartData] = useState(null);
  const [maxYAxis, setMaxYAxis] = useState(0);
    const formatDateToYYYYMMDD = (date) => {
        const year = date.getFullYear();
...
const Dashboard = () => {
  ...
  useEffect(() => {
    setLoading(true); 
    axios.get(`${frontendconfig.backendUrl}/resource_allocations_on_project`)
      .then((response) => {
  ...
  useEffect(() => {
    if (selectedIntake) {
      const filtered = intakeData.filter(item => item.intake_number === selectedIntake);
  ...
  useEffect(() => {
    if (selectedUser) {
      const filtered = userData.filter(item => item.resource_name === selectedUser);
  ...
  useEffect(() => {
    ...
    if (selectedIntake) {
      filtered = filtered.filter(item => item.intake_number === selectedIntake);
    }
    if (startDate) {
  ...
  useEffect(() => {
    ...
    if (selectedUser) {
      filtered = filtered.filter(item => item.resource_name === selectedUser);
    }
    if (startDate) {
      filtered = filtered.filter(item => new Date(item.start_date) >= new Date(startDate));
    }
    ...
    setFilteredUserData(filtered);
  }, [selectedUser, startDate, endDate, userData]);
  ...
  return (
    <div className="dashboard-resource-allocation">
      ...
      <div align>
            <div className="form-type-row " display="justify">
              <div><label>Select Start Date Range:</label></div>
              <div><input type="date" className="select-form-type-row"   id="start_date" name="start_date" value={startDate} onChange={handleStartDateChange} /></div>
              <div> </div>
              <div><label>Select Start Date Range:</label></div>
        ...
        <div className="charts-container">
          <div className="chart-column">
            <div className="form-type-row ">
              ...
              <select id="intake" className="select-form-type-row"   value={selectedIntake} onChange={handleIntakeChange}>
                <option value="">All</option>
                {[...new Set(intakeData.map(item => item.intake_number))].map(intake => (
                  <option key={intake} value={intake}>{intake}</option>
                ))}
              </select>
            </div>
            <h5>Intake level Chart</h5>
            <div className="chart-box">
              <ResourceUtilizationAllocation chartType="Line" data={filteredData} />
            </div>
            <h5></h5>
            <div className="chart-box">
              <ResourceUtilizationAllocation chartType="Bar" data={filteredData} />
            </div>
            <h5></h5>
            <div className="chart-box pie-chart-box">
              <ResourceUtilizationAllocation chartType="Pie" data={filteredData} />
            </div>
          </div>
          <div className="chart-column">
            <div className="form-type-row">
              <label htmlFor="user">Select Resource: </label>
              <select id="user" className="select-form-type-row"   value={selectedUser} onChange={handleUserChange}>
                <option value="">All</option>
                {[...new Set(userData.map(item => item.resource_name))].map(user => (
            ...
            <div className="chart-box">
              <ResourceUtilizationCapacity chartType="Line" data={filteredUserData}  />
            </div>
            <h5></h5>
            <div className="chart-box">
              <ResourceUtilizationCapacity chartType="Bar" data={filteredUserData} />
            </div>
            <h5></h5>
            <div className="chart-box pie-chart-box">
              <ResourceUtilizationCapacity chartType="Pie" data={filteredUserData}  />
            </div>
          </div>
        </div>
      )}
