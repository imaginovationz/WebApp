...
export default function ReleaseQEHome({ release }) {
  ...
  // Local-first file + sheet to load
  ...
  //  Optional SharePoint fallback for inline viewing if local load fails
  const spBase =
  ...
  // Load workbook from local /files, extract sheet as AoA with formulas
  useEffect(() => {
    let cancelled = false;
    (async () => {
      setLoading(true);
      ...
      try {
        ...
        const data = Array.from({ length: nrows }, (_, r) =>
          Array.from({ length: ncols }, (_, c) => {
            ...
            return cell.v ?? null;
                ...
                //  Map Excel merges to Handsontable's mergeCells config
        const merges = Array.isArray(ws['!merges'])
          ...
          : [];
        //  Column widths (Excel -> pixels). Prefer wpx, else convert wch.
       ...
       const toPxFromWpx = (wpx) => Math.max(60, Math.min(500, Math.floor(wpx)));
        ...
        //  Row heights (best-effort if present)
  ...
  //Set up / reset HyperFormula engine when data changes
  useEffect(() => {
