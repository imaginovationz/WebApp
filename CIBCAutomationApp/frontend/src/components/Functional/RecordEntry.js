...
const RecordEntry = () => {
  ...
  useEffect(() => {
    // Fetch the release values from the backend functionalfrontendconfig
  ...
  const handleResourceSubmit = (e) => {
    ...
    axios.post(`${functionalfrontendconfig.backendUrl}/resources`, resourceData)
      .then(response => {
  ...
  const handleProjectSubmit = (e) => {
    ...
    axios.post(`${functionalfrontendconfig.backendUrl}/projects`, projectData)
      .then(response => {
  ...
  const handleProjectTimelineSubmit = (e) => {
    ...
    axios.post(`${functionalfrontendconfig.backendUrl}/projects_timeline`, updatedTimelineData)
      .then(response => {
  ...
  const handleIntakeNumberChange = (e) => {
    ...
    axios.get(`${functionalfrontendconfig.backendUrl}/projects_timeline/${selectedIntake.split(' ')[0]}`)
      .then(response => {
        if (response.data) {
  ...
  const handleResourceAllocationSubmit = async (e) => {  // major change here
    ...
    try {
      ...
      if (match) {
            ...
            const response = await axios.get(`${functionalfrontendconfig.backendUrl}/resources/searchNameInfo?name=${encodeURIComponent(resourceName)}&info=${encodeURIComponent(resourceInfo)}`);
            ...
            for (const record of records) {
              console.log(record);
              await axios.post(`${functionalfrontendconfig.backendUrl}/resource_allocations`, record)
              .then(response => { 
              setErrorMessage(''); // Clear any previous error messages
              })
              .catch(error => {
