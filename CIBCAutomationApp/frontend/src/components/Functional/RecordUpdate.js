...
const RecordUpdate = ({ headers: initialHeaders }) => {
    ...
    useEffect(() => {
        ...
        axios.get(`${functionalfrontendconfig.backendUrl}/applications`)
            .then(res => {
                setApplications(res.data);
            })
            .catch(() => setApplications([]));

        axios.get(`${functionalfrontendconfig.backendUrl}/resources`)
            .then(res => {
        ...
        // Fetch the manager resources from the backend
        axios.get(`${functionalfrontendconfig.backendUrl}/resources/project-managers`)
            .then(response => {
                console.log('Fetched manager resources:', response.data); // Log the fetched data
                setManagerResources(response.data.map(resource => `${resource.name} -  ${resource.resource_additional_info} (${resource.info})`));
            })
            .catch(error => {
        ...
        axios.get(`${functionalfrontendconfig.backendUrl}/releases`)
            .then(response => {
                setReleases(
    ...
    const handleLoginSubmit = (userid, password) => {
        ...
        const validateCredentials = async () => {
            try {
                const response = await axios.post(`${functionalfrontendconfig.backendUrl}/login`, { userid, password });
    ...
    const fetchRecordData = (type) => {
        ...
        axios.get(`${functionalfrontendconfig.backendUrl}/${type}`)
            .then(response => {
    ...
    const applySorting = (data) => {
        ...
        return [...data].sort((a, b) => {
            ...
            if (sortColumn === 'intake_entry_date' || sortColumn === 'timestamp') {
                // Date sorting
                aValue = new Date(aValue || '1900-01-01');
                bValue = new Date(bValue || '1900-01-01');
            } else if (sortColumn === 'release') {
                // Release sorting - handle text comparison
                aValue = aValue ? aValue.toString().toLowerCase() : '';
                bValue = bValue ? bValue.toString().toLowerCase() : '';
            } else {
                // General text sorting
                aValue = aValue ? aValue.toString().toLowerCase() : '';
                bValue = bValue ? bValue.toString().toLowerCase() : '';
            }
            ...
            if (record.name && record.company_name && record.location && record.intake_number) {
                ...
                Object.entries(record).forEach(async ([name, value]) => { // Use async function inside forEach
                    if (name !== 'name' && name !== 'company_name' && name !== 'location' && name !== 'intake_number') {
                        ...
                        try {
                            const resource_response = await axios.get(`${functionalfrontendconfig.backendUrl}/resources/searchNameCompanyLocation?name=${encodeURIComponent(record.name)}&location=${encodeURIComponent(record.location)}&company_name=${encodeURIComponent(record.company_name)}`); 
                            console.log('Response:', resource_response.data.resource_id);
                            const resource_id = resource_response.data.resource_id;
                            
                            const delete_response = await axios.delete(`${functionalfrontendconfig.backendUrl}/resource_allocations`, {
                                    params: {
                            ...
                            const insert_response = await axios.post(`${functionalfrontendconfig.backendUrl}/resource_allocations`, {
                                resource_id: resource_id,
    ...
    const renderTable = () => {
        ...
        console.log('viewType:', viewType); // Debugging: Check the value of viewType
        const filteredData = applySorting(applyFilters(recordData));

        // Define which columns to always show regardless of date filtering
        // const alwaysShowColumns = ['name', 'company_name', 'location','intake_number'];
        const alwaysShowColumns = ['intake_number', 'name', 'company_name', 'location','intake_number','intake_name', 'release','domain','project_status','functional_qe_lead','automation_qe_lead','sme_consultant','qa_manager','intake_entry_date','timestamp'];
                    ...
                    <thead >
                        <tr>
                            {filteredHeaders.map((header, index) => {
                                const isSortableColumn = ['intake_entry_date', 'release', 'timestamp'].includes(header);
                                const isSorted = sortColumn === header;
                                
                                return (
                                    <th key={index} 
                                        onClick={isSortableColumn ? () => handleSort(header) : undefined}
                                        style={isSortableColumn ? { cursor: 'pointer', userSelect: 'none' } : {}}
                                    >
    ...
    const ProjectUpdateModal = ({ show, project, onClose, onSave }) => {
        ...
        const [showAllManagers, setShowAllManagers] = useState({});
        ...
        const keys = project
            ? Object.keys(project).filter(
                key => key !== 'updated_by' && key !== 'timestamp' && key !== 'intake_number'
            )
            : [];
        
        // Move project_additional_info to the end so it appears in the last row
        const reorderedKeys = keys.filter(key => key !== 'project_additional_info');
        if (keys.includes('project_additional_info')) {
            reorderedKeys.push('project_additional_info');
        }
        
        const fieldTriplets = [];
        for (let i = 0; i < reorderedKeys.length; i += 3) {
            fieldTriplets.push(reorderedKeys.slice(i, i + 3));
        }
                        ...
                        {fieldTriplets.map((triplet, rowIdx) => (
                            ...
                            >
                                {triplet.map((key) => {
                                    ...
                                    if (key === 'functional_qe_lead' || key === 'automation_qe_lead' || key === 'sme_consultant') {
                                        const currentValue = formData[key] || '';
                                        let options = [];
                                    ...
                                    if (key === 'qa_manager') {
                                        const currentValue = formData[key] || '';
                                        let options = [];
                                        if (showAllManagers[key]) {
                                            // Show all manager resources
                                            options = (managerResources || [])
                                                .map(manager => ({
                                                    value: manager,
                                                    label: manager
                                                }))
                                        ...
                                        return (
                                            <div key={key} style={{ flex: 1 }}>
                                                ...
                                                <select
                                                    ...
                                                    onChange={e => {
                                                        if (e.target.value === "__other__") {
                                                            setShowAllManagers(prev => ({ ...prev, [key]: true }));
                                                        } else {
                                                            setFormData({ ...formData, [key]: e.target.value });
                                                        }
                                                    }}
    ...
    const handleSaveProject = async (updatedProject) => {
        ...
        try {
            ...
            await axios.put(`${functionalfrontendconfig.backendUrl}/projects/${intake_number}`, filteredData);
