...
@app.route('/ecif_sim_customer_info', methods=['GET'])
def get_ecif_sim_customer_info():
    try:
        result = db.session.execute("SELECT Environment,ecifID,firstName,lastName,knownAsName,dateOfBirth,isDeceased,gender,language,residency_country,residency_province,firstServiceDate,isAboriginal,customerType,partyOrigination,address_street,address_city,address_provinceState,address_postalCode,address_country,addressLine1,addressLine2,isPOBox,movedInDate,addressStatus,phoneType,phoneContext,phoneCountry,areaCode,phoneNumber,emailContext,emailAddress FROM ecif_sim_customer_info")
        rows = result.fetchall()
        data = [dict(row) for row in rows]
        return jsonify(data)
...
def ensure_dynamic_model(schema_row: UISchema):
    ...
    fields = schema.get('fields', [])
    # Basic validation & normalization for fields
    if not isinstance(fields, list):
        raise ValueError('Schema fields must be a list')
    seen = set()
    normalized = []
    reserved = {'id'}  # reserve primary key name
    for f in fields:
        if not isinstance(f, dict):
            continue
        raw_name = f.get('name') or ''
        fname = re.sub(r'[^a-zA-Z0-9_]', '_', raw_name.strip())[:60]
        if not fname:
            continue
        if fname in reserved:
            raise ValueError(f'Field name "{fname}" is reserved')
        if fname in seen:
            raise ValueError(f'Duplicate field name "{fname}"')
        seen.add(fname)
        ftype = (f.get('type') or 'string').lower()
        # Honor an optional maxLength (cap at 191 to avoid MySQL row size / index issues)
        try:
            max_len = int(f.get('maxLength', 191))
        except Exception:
            max_len = 191
        max_len = max(1, min(max_len, 191))
        f['__normalized_name'] = fname
        f['__normalized_type'] = ftype
        f['__normalized_length'] = max_len
        normalized.append(f)
    fields = normalized
    # Safeguard: limit excessive column counts (pragmatic upper bound)
    if len(fields) > 100:
        raise ValueError('Too many columns (>100) in schema')
    ...
    for f in fields:
        fname = f['__normalized_name']
        ftype = f['__normalized_type']
        required = f.get('required', False)
        length = f['__normalized_length']
        base_type = type_map.get(ftype, db.String(length))
        # Adjust default string length to computed length
        if ftype in ('string', 'varchar'):
            coltype = db.String(length)
        elif ftype == 'text':
            coltype = db.Text
        else:
            coltype = base_type
    ...
    try:
        ...
        # Provide more actionable error details (e.g., MySQL row size / duplicate keys)
        app.logger.exception(f'Failed creating dynamic table {table_name}')
        raise ValueError(f'Dynamic table creation failed: {e}')
...
def publish_schema(name):
    ...
    try:
        ...
        app.logger.exception('Unexpected error publishing schema')
...
def list_schema_data(schema_name):
    ...
    # Build raw->normalized mapping so we can echo raw names expected by UI
    def _norm(n: str):
        return re.sub(r'[^a-zA-Z0-9_]', '_', (n or '').strip())[:60]
    raw_to_norm = {}
    for f in schema_def.get('fields', []):
        raw = f.get('name')
        if not raw:
            continue
        raw_to_norm[raw] = _norm(raw)
    records = model.query.all()
    output = []
    for rec in records:
        base = rec.to_dict()
        # Duplicate values under raw keys if different, preserving existing normalized keys
        for raw, norm in raw_to_norm.items():
            if raw != norm and norm in base and raw not in base:
                base[raw] = base[norm]
        output.append(base)
...
def create_schema_record(schema_name):
    ...
    try:
        ...
        def _norm(n: str):
            import re
            return re.sub(r'[^a-zA-Z0-9_]', '_', (n or '').strip())[:60]
        # Map raw name -> definition; also keep normalized name
        field_defs = {}
        for f in schema_def.get('fields', []):
            raw = f.get('name')
            if not raw:
                continue
            field_defs[raw] = f
            f['__normalized_name'] = _norm(raw)
        payload = request.get_json() or {}
        app.logger.info(f'Dynamic insert received raw payload for {schema_name}: {payload}')
        data = {}
        # Accept either raw or normalized keys in payload
        inserted_columns = []
        for raw_name, fdef in field_defs.items():
            norm_name = fdef['__normalized_name']
            stripped = raw_name.strip()
            # Build candidate keys to search in payload
            candidates = [raw_name]
            if stripped not in candidates:
                candidates.append(stripped)
            if norm_name not in candidates:
                candidates.append(norm_name)
            # Also add lowercase variants (frontend sometimes lowercases on submit)
            ...
            for c in candidates:
                if c in payload:
                    val = payload[c]
                    break
            # Treat placeholder value (exact field name) as empty for non-required fields
            if isinstance(val, str) and val.strip() in {raw_name.strip(), norm_name.strip()} and not fdef.get('required'):
                val = ''
            if fdef.get('required') and (val is None or (isinstance(val, str) and val.strip() == '')):
                return jsonify({'error': f'Missing required field {raw_name}'}), 400
            coerced = _coerce_field(fdef.get('type','string').lower(), val)
            if fdef.get('required') and coerced is None:
                return jsonify({'error': f'Invalid or missing value for required field {raw_name}'}), 400
            # Use normalized key to match actual column name
            data[norm_name] = coerced
            inserted_columns.append(norm_name)
            app.logger.debug(f'Dynamic insert field {raw_name} candidates={candidates} chosen_value={val!r} stored_as={norm_name} coerced={coerced!r}')
        # Warn if payload contains unknown keys (optional diagnostic)
        for k in payload.keys():
            if k not in field_defs and k not in {f['__normalized_name'] for f in field_defs.values()}:
                app.logger.warning(f'Ignoring unknown field "{k}" in dynamic insert for {schema_name}')
        app.logger.info(f'Dynamic insert schema={schema_name} raw_keys={list(payload.keys())} mapped_columns={inserted_columns}')
        obj = model(**data)
        db.session.add(obj)
        db.session.commit()
        record = obj.to_dict()
        # Echo back raw payload vs mapped to help frontend debugging
        return jsonify({
            'record': record,
            'inserted_columns': inserted_columns,
            'null_columns': [c for c,v in record.items() if v is None],
...
def update_schema_record(schema_name, record_id):
    ...
    for raw_name, fdef in field_defs.items():
        norm_name = fdef['__normalized_name']
        candidates = [raw_name, raw_name.strip(), norm_name, raw_name.lower(), norm_name.lower()]
        val = None
        for c in candidates:
            if c in payload:
                val = payload[c]
        ...
        if val is None:
            continue
        if fdef.get('required') and (isinstance(val,str) and val.strip()==''):
            return jsonify({'error': f'Missing required field {raw_name}'}), 400
        coerced = _coerce_field(fdef.get('type','string').lower(), val)
        if fdef.get('required') and coerced is None:
            return jsonify({'error': f'Invalid value for required field {raw_name}'}), 400
        before = getattr(obj, norm_name, None)
        setattr(obj, norm_name, coerced)
        after = getattr(obj, norm_name, None)
        changed[norm_name] = {'before': before, 'after': after}
    db.session.commit()
    result = obj.to_dict()
    return jsonify({'record': result, 'changed': changed})
